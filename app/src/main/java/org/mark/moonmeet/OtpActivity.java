...
public class OtpActivity extends AppCompatActivity {
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		country_text.setOnClickListener(_view -> {
			...
			toContinue.setClass(getApplicationContext(), CountrycodeActivity.class);
			toContinue.setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
		...
		Pauth = new PhoneAuthProvider.OnVerificationStateChangedCallbacks(){
			...
			public void onCodeSent(@NonNull String _verificationId, @NonNull PhoneAuthProvider.ForceResendingToken _token) {
				...
				Verification = true;
				phone = plus.getText().toString().concat(dial_code.getText().toString().concat(" ".concat(number_edittext.getText().toString())));
			...
			if (_success) {
				...
				if (isNewUser) {
					while (true) {
						...
						if (!taked_usernames.contains(FirebaseAuth.getInstance().getCurrentUser().getUid())) {
							Map = new HashMap<>();
							...
							sp_mydt.edit().putString("phone", phone).apply();
							sp_mydt.edit().putString("username", username).apply();
							sp_mydt.edit().putString("country", country_text.getText().toString()).apply();
							toContinue.setClass(OtpActivity.this.getApplicationContext(), SetupActivity.class);
							toContinue.putExtra("taked_photo", ".");
							OtpActivity.this.startActivity(toContinue);
							break;
						}
					}
				} else {
					toContinue.setClass(OtpActivity.this.getApplicationContext(), LaunchActivity.class);
					OtpActivity.this.startActivity(toContinue);
				}
			} else {
	...
	private void initializeLogic() {
		...
		if (getIntent().getStringExtra("Country").equals(".") && getIntent().getStringExtra("Code").equals(".")) {
			...
			country_text.setText(getIntent().getStringExtra("Country"));
			dial_code.setText(getIntent().getStringExtra("Code"));
		}
						...
						timer = new TimerTask() {
							...
							public void run() {
								runOnUiThread(() -> {
									...
									extra2.setDuration((int)(400));
									extra2.start();
									sp_df.edit().remove("cancelled").apply();
								});
							}
						};
						_timer.schedule(timer, (int)(200));
