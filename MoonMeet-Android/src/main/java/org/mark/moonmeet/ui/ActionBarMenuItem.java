...
import org.mark.moonmeet.components.ActionBarPopupWindow;
import org.mark.moonmeet.components.CloseProgressDrawable2;
import org.mark.moonmeet.components.EditTextBoldCursor;
...
*/
    ...
    public ActionBarMenuItem setIsSearchField(boolean value, final boolean wrapInScrollView) {
        ...
        if (value && searchContainer == null) {
            ...
            clearButton = new androidx.appcompat.widget.AppCompatImageView(getContext()) {
                @Override
            ...
            clearButton.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (searchField.length() != 0) {
                        searchField.setText("");
                    } else if (ActionBarMenuItem.this.hasRemovableFilters()) {
                        searchField.hideActionMode();
                   /* for (int i = 0; i < currentSearchFilters.size(); i++) {
            ...
            clearButton.setContentDescription("Clear");
            if (wrapInScrollView) {
        ...
        if (clearButton != null) {
            if (!hasRemovableFilters() && TextUtils.isEmpty(searchField.getText()) &&
                ...
                if (clearButton.getTag() != null) {
                    ...
                    if (animateClear) {
                        clearButton.animate().setInterpolator(new DecelerateInterpolator()).alpha(0.0f).setDuration(180).scaleY(0.0f).scaleX(0.0f).rotation(45).withEndAction(new Runnable() {
                            @Override
                            public void run() {
                                clearButton.setVisibility(INVISIBLE);
                            }
