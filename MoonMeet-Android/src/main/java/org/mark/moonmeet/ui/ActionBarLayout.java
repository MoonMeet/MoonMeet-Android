...
public class ActionBarLayout extends FrameLayout {
	...
	public interface ActionBarLayoutDelegate {
		...
		void onRebuildAllFragments(ActionBarLayout layout, boolean all);
	}
	
	public class LinearLayoutContainer extends FrameLayout {
	...
	protected AppCompatActivity parentActivity = null;
	...
	public void movePreviewFragment(float dy) {
		...
		if (nextTranslation > 0) {
			...
			final BaseFragment fragment = fragmentsStack.get(fragmentsStack.size() - 1);
...
return false;
	...
	private void checkNeedRebuild() {
		...
		} /*lse if (animateThemeAfterAnimation) {
...
*/
	...
	public boolean presentFragment(final BaseFragment fragment, final boolean removeLast, boolean forceWithoutAnimation, boolean check, final boolean preview) {
		...
		fragment.onDelayedTransition(true);
		containerViewBack.addView(fragmentView);
		//fragment.onCustomTransitionAnimation(true, () -> 
		...
		if (needAnimation || preview) {
			if (useAlphaAnimations && fragmentsStack.size() == 1) {
				...
				if (!preview) {
					animation = fragment.onCustomTransitionAnimation(true, () -> ActionBarLayout.this.onAnimationEndCheck(false));
				}
				...
				if (animation == null) {
					...
					if (containerView.isKeyboardVisible || containerViewBack.isKeyboardVisible) {
						...
						AndroidUtilities.runOnUIThread(waitingForKeyboardCloseRunnable, true? 250 : 200); //if smooth keyboard
					} else if (fragment.needDelayOpenAnimation()) {
						delayedOpenAnimationRunnable = new Runnable() {
							@Override
							public void run() {
	...
	public void closeLastFragment(boolean animated) {
		...
		setInnerTranslationX(0);
		boolean needAnimation = inPreviewMode || transitionAnimationPreviewMode || animated && true;//MessagesController.getGlobalMainSettings().getBoolean("view_animations", true);
			...
			if (useAlphaAnimations) {
				...
				onCloseAnimationEndRunnable = new Runnable() {
					...
					public void run() {
						...
						ActionBarLayout.this.setVisibility(GONE);
						if (backgroundView != null) {
