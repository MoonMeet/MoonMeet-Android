...
public class XiaomiUtilities {
        ...
        public static boolean isCustomPermissionGranted(int permission) {
            try {
                AppOpsManager mgr = (AppOpsManager) MoonMeetApplication.applicationContext.getSystemService(Context.APP_OPS_SERVICE);
                Method m = AppOpsManager.class.getMethod("checkOpNoThrow", int.class, int.class, String.class);
                int result = (int) m.invoke(mgr, permission, android.os.Process.myUid(), MoonMeetApplication.applicationContext.getPackageName());
        ...
        public static Intent getPermissionManagerIntent() {
            ...
            intent.putExtra("extra_pkgname", MoonMeetApplication.applicationContext.getPackageName());
            return intent;
        }
}
